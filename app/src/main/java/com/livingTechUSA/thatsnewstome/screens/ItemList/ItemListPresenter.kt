...
class ItemListPresenter(
    ...
    override fun onCreated() {
        ...
        launch(appDispatchers.io()) {
            initHeadlineList()?.articles?.let { mModel.addArticles(it) }
    ...
    private suspend fun getArticles() {
        ...
        try {
            call?.articles?.let { updateList(it) }
        } catch (e:Exception){
           e.printStackTrace()
...
private fun ItemListModel.changeSearchQueryListenerState(enable: Boolean) {
